...
namespace MyControls
{
    ...
    public partial class AGauge : UserControl
    {
        ...
        //private Point m_Center = new Point(100, 100);//中心点靠左，刻度值有时候会显示不全
        private Single m_MinValue = -100;
        private Single m_MaxValue = 400;
        private Point m_Center = new Point(105, 100);
        //private Single m_MinValue = 0;
        //private Single m_MaxValue = 1;
        ...
        //新添加了CapText的字体控制属性
        Font m_CapFont=new Font("宋体", 15);//要初始化才行
        [System.ComponentModel.Browsable(true),
        System.ComponentModel.Category("AGauge"),
        System.ComponentModel.Description("CapText字体")]
        public Font CapFont  //cap标签的字体设置，新加
        {
            get
            {
                return m_CapFont;
        ...
        [System.ComponentModel.Browsable(true),
        System.ComponentModel.Category("AGauge"),
        System.ComponentModel.Description("The value.")]//当前指示值
        public Single Value
        {
            get
            {
            ...
            set
            {
                if (m_value != value)//当前值发生变化时才执行，否则直接跳过
                {
                    m_value = Math.Min(Math.Max(value, m_MinValue), m_MaxValue);//确保取值不大于最大值，不小于最小值
                    ...
                    for (Int32 counter = 0; counter < NUMOFRANGES - 1; counter++)//逐一刷新四个Range块
                    {
        ...
        public Byte Cap_Idx    //便签、文本标记序号，一共0-4，5个
        {
            get
            {
                return m_CapIdx;
        ...
        //private Color CapColor  //私有属性不可设置颜色
        {
            get
            {
        ...
        //public Color[] CapColors//上面设置不显示了，没有必要设置公共属性
        ...
        public Single MinValue
        {
            ...
            set
            {
                ...
                && (value < m_MaxValue))
                {
                    m_MinValue = value;
                    drawGaugeBackground = true;
                    m_ScaleLinesMajorStepValue = (m_MaxValue - m_MinValue) / 10;//默认调整刻度步长为总数的十分之一，后加
                    Refresh();
                }
            }
        }
        ...
        {
            //find upper and lower bounds for numeric characters  找到上下限为数字字符
            Int32 c1;
            Int32 c2;
            Boolean boundfound;
            Bitmap b;
            Graphics g;
            SolidBrush backBrush = new SolidBrush(Color.White);//背景画刷
            SolidBrush foreBrush = new SolidBrush(Color.Black);//前景画刷
            SizeF boundingBox;

            b = new Bitmap(5, 5);//位图对象初始化
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (drawGaugeBackground)
            {
                ...
                ggr.FillRectangle(new SolidBrush(BackColor), ClientRectangle);//填充背景色
                ...
                }   //画弧线               画基本弧线的画笔              画图矩形区     中心点减去圆弧半径                                                                                         开始弧度         弧度                  

                String valueText = "";
                SizeF boundingBox;
                Single countValue = 0.0F;
                ...
                while (countValue <= (m_MaxValue - m_MinValue))
                {
                    ...
                    if (m_ScaleNumbersRotation != 0)
                    {
                        ...
                        //ggr.RotateTransform(90 + m_BaseArcStart + countValue * m_BaseArcSweep / (m_MaxValue - m_MinValue));
                    }
                    ...
                    ////////////////////////
                    countValue =(float) Math.Round((double)countValue, 3);
                    //#region 新加，处理精度问题，不然产生累积误差
                    //Int32 tempcountValue = (Int32)(countValue * 1000);//取小数点后三位
                    //countValue = (Single)tempcountValue / 1000;
                    //#endregion
                    counter1++;
                }
            ...
            switch (m_NeedleType)
            {
                #region 指针样式0
                case 0: //指针模式0，尖锥形
                    PointF[] points = new PointF[3];//声明三个浮点数的点
                    Brush brush1 = Brushes.White;   //声明四个画刷
                    Brush brush2 = Brushes.White;
                    Brush brush3 = Brushes.White;
                    Brush brush4 = Brushes.White;

                    Brush brushBucket = Brushes.White;//画刷桶，用于交换变量的临时变量？
                    Int32 subcol = (Int32)(((brushAngle + 225) % 180) * 100 / 180);//没看懂
                    ...
                    //填充指针盘                                    指针盘                                   起点                                            矩形区大小
                    #region 绘制指针盘边框
                    switch (m_NeedleColor1)//绘制指针盘的边框颜色也同样是指针的颜色
                    {
                    ...
                    points[0].X = (Single)(Center.X + m_NeedleRadius * Math.Cos(needleAngle));//指针尖端点
                    points[0].Y = (Single)(Center.Y + m_NeedleRadius * Math.Sin(needleAngle));
                    points[1].X = (Single)(Center.X - m_NeedleRadius / 20 * Math.Cos(needleAngle));//指针中间凸点
